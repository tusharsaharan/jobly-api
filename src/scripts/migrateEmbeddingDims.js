...
async function reembedMismatched() {
  if (!hasValidKey()) {
    console.warn("⚠️  GEMINI_API_KEY not valid — cannot re-embed. Falling back to zero-vector patch.");
...
async function main() {
  ...
  if (doWipe) {
    console.log("⚠️  --wipe: delegating to seedCompleteStudyRAG full re-seed (drops study_resource)...");
  ...
  if (badAfter.length > 0) {
    console.warn("⚠️  Still have non-3072 dims after migration:", badAfter);
    process.exitCode = 1;
  } else {
