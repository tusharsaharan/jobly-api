...
async function bootstrap() {
 ...
 if (dim!== 3072) {
 logger.warn({ total, dim }, ` EmbeddingChunk dims are ${dim}, expected 3072. Run "npm run migrate:embeddings" or "npm run seed:rag" to normalize. Hybrid RRF fallback is active, but Atlas vector search will fail until migrated.`);
 } else {
 ...
 if (bad > 0) logger.warn({ sampleBad: bad, total }, " Some EmbeddingChunk dims!= 3072 — migrate recommended.");
