 ...
 * (any single "-latest" alias can 503 for minutes — never hardcode one model).
 */
async function geminiText(prompt, { json = true } = {}) {
  const key = process.env.GEMINI_API_KEY;
  if (!key) throw new Error("GEMINI_API_KEY not set");
  return generateWithFallback(key, {
    contents: prompt,
    config: json ? { responseMimeType: "application/json" } : undefined,
  });
}

// Legacy alias kept for any external callers of getAI()
function getAI() {
  throw new Error("getAI() is deprecated — use geminiText() which carries the model fallback chain");
}
